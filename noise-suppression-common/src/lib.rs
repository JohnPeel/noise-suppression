...
impl NoiseSuppression {
    pub fn set_grace_periods(&mut self, grace_periods: usize) {
        ...
        self.remaining_grace_periods =
            usize::min(self.remaining_grace_periods, self.initial_grace_periods);
    }
    ...
    pub fn process(&mut self, output: &mut [f32], input: &[f32], sample_count: usize) {
        self.input_buffer
            .extend(input.iter().map(|f| i16::from_sample(*f) as f32));
        ...
        self.output_buffer
            .extend(vec![f32::EQUILIBRIUM; frames_to_process]);

        for i in 0..samples_to_process {
            let output_buffer = &mut self.output_buffer
                [output_start + i * FRAME_SIZE..output_start + i * FRAME_SIZE + FRAME_SIZE];
            let vad_probability = {
            ...
            if !self.first {
                ...
                if self.remaining_grace_periods > 0 {
                    ...
                    output_buffer
                        .iter_mut()
                        .for_each(|f| *f = f32::from_sample(*f as i16));
                } else {
        ...
        if frames_to_copy < sample_count {
            output[frames_to_copy..sample_count]
                .iter_mut()
